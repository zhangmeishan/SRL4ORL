...
class BiLSTMModel(nn.Module):
    ...
    def __init__(self, vocab, config, pretrained_embedding):
        ...
        self.srltag_embed = nn.Embedding(vocab.srlvocab_size, config.srltag_dims, padding_idx=self.PAD)
        nn.init.normal(self.srltag_embed.weight, 0.0, 1.0 / (config.srltag_dims ** 0.5))
        ...
        self.input_dims = config.word_dims + config.srltag_dims + config.predict_dims
    ...
    def forward(self, words, extwords, srltags, predicts, inmasks):
        ...
        x_srltag_embed = self.srltag_embed(srltags)
        ...
        embeddings = torch.cat((x_embed, x_srltag_embed, x_predict_embed), 2)

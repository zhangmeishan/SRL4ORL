...
def train(data, dev_data, test_data, labeler, vocab, srl_vocab, config):
    optimizer = Optimizer(filter(lambda p: p.requires_grad, labeler.model.parameters()), config) \
        if config.parser_tune == 0 else \
        Optimizer(filter(lambda p: p.requires_grad, \
                         list(labeler.model.parameters()) + list(labeler.srl_model.parameters())), config)
    ...
    for iter in range(config.train_iters):
        ...
        for onebatch in data_iter(data, config.train_batch_size, True):
            ...
            srl_words, srl_extwords, srl_predicts, srl_inmasks = \
                batch_data_variable(onebatch, vocab, srl_vocab)
            labeler.model.train()

            labeler.forward(words, extwords, predicts, inmasks, \
                            srl_words, srl_extwords, srl_predicts, srl_inmasks)
            ...
            if batch_iter % config.validate_every == 0 or batch_iter == batch_num:
                gold_num, predict_num, correct_num = \
                    evaluate(dev_data, labeler, vocab, srl_vocab, config.dev_file + '.' + str(global_step))
                ...
                test_gold_num, test_predict_num, test_correct_num = \
                    evaluate(test_data, labeler, vocab, srl_vocab, config.test_file + '.' + str(global_step))
...
def evaluate(data, labeler, vocab, srl_vocab, outputFile):
    start = time.time()
    labeler.model.eval()
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--srl_config_file', default='examples/srl.cfg')
    argparser.add_argument('--thread', default=4, type=int, help='thread num')
    argparser.add_argument('--use-cuda', action='store_true', default=True)

    args, extra_args = argparser.parse_known_args()
    config = Configurable(args.config_file, extra_args)
    srl_config = SRLConfigurable(args.srl_config_file)

    srl_vocab = pickle.load(open(srl_config.load_vocab_path, 'rb'))
    srl_vec = srl_vocab.create_pretrained_embs(srl_config.pretrained_embeddings_file)

    srl_model = SRLBiLSTMCRFModel(srl_vocab, srl_config, srl_vec)
    srl_model.load_state_dict(torch.load(srl_config.load_model_path, map_location=lambda storage, loc: storage))
    ...
    model = eval(config.model)(vocab, config, vec, 2 * srl_config.lstm_hiddens)
    if config.use_cuda:
        torch.backends.cudnn.enabled = True
        model = model.cuda()
        srl_model = srl_model.cuda()

    labeler = SRLLabeler(model, srl_model)

    data = read_corpus(config.train_file)
    dev_data = read_corpus(config.dev_file)
    test_data = read_corpus(config.test_file)

    train(data, dev_data, test_data, labeler, vocab, srl_vocab, config)
